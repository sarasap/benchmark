...
Differences from Exa's pipeline: Exa used Browserbase for rendering and an unspecified model for markdown generation. 
...
Note on absolute scores: Scores differ from Exa's published results. This is expected given the different golden reference pipeline and different judge model (Gemini 2.5 Pro vs GPT-5.4). The relative ranking between providers is the more meaningful comparison.
...
In rag_020, the query asks which Go version first supported protobuf option retention (expected: "1.29.0"). Parallel retrieved the correct documentation page containing the exact sentence. Exa's retrieved pages were related but did not fetch the expected answer. Tavily fetched pages from the right domain, but its summarized content did not include the specific version number.


Results when Exa is configured to return only query-relevant highlights and the max_character parameter is removed:
...
Agentic mode holds quality essentially constant while dropping tokens from 1137 to 437, landing right alongside Exa-highlights' 458. So once both providers are run in their token-efficient configurations, the efficiency gap largely closes, and the providers are comparable in cost.
